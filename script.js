document.addEventListener('DOMContentLoaded', () => {
    ...
    scrambleArea.addEventListener('click', (e) => {
        ...
        if (letterDiv && answer.length < word.length) {
    ...
    answerArea.addEventListener('drop', (e) => {
        ...
        if (targetBox && !targetBox.textContent && answer.length < word.length) {
            // 找到被拖曳的原始元素
            const originalLetterDiv = document.querySelector(`.scramble-letter[data-letter="${droppedLetter}"]:not(.hidden)`);
    ...
    function moveBackToScrambleArea(boxDiv) {
        ...
        const originalLetterDiv = document.querySelector(`.scramble-letter[data-letter="${letter}"].hidden`);
        
        if (originalLetterDiv) {
            originalLetterDiv.classList.remove('hidden');
            boxDiv.textContent = '';
            boxDiv.dataset.letter = '';
